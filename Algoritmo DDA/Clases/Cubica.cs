...
namespace Algoritmo_DDA.Clases
{
    internal class Cubica
    {
        ...
        public void Draw(Graphics g)
        {
            ...
            if (point4.HasValue)
            {
                DrawControlPoint(g, point4.Value, Brushes.Orange);
            }

            if (point1.HasValue && point2.HasValue && point3.HasValue && point4.HasValue)
            {
                DrawCubicBezierCurveAnimated(g, point1.Value, point2.Value, point3.Value, point4.Value, currentL);
            }
        }
        ...
        public void SelectPointAtLocation(Point location)
        {
            ...
            else if (point4.HasValue && IsPointSelected(point4.Value, location))
            {
        ...
        private void DrawCubicBezierCurveAnimated(Graphics g, PointF p0, PointF p1, PointF p2, PointF p3, float maxL)
        {
            using (Pen curvePen = new Pen(Color.Purple, 2))
            {
                for (float l = 0; l <= maxL; l += 0.01f)
                {
                    float x = (float)(Math.Pow(1 - l, 3) * p0.X +
                                    3 * Math.Pow(1 - l, 2) * l * p1.X +
                                    3 * (1 - l) * l * l * p2.X +
                                    Math.Pow(l, 3) * p3.X);

                    float y = (float)(Math.Pow(1 - l, 3) * p0.Y +
                                    3 * Math.Pow(1 - l, 2) * l * p1.Y +
                                    3 * (1 - l) * l * l * p2.Y +
                                    Math.Pow(l, 3) * p3.Y);
